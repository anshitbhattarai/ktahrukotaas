...
export default function Table({params}:{params:Promise<{id:string}>}){
  ...
  async function start(){
    ...
    if(room.game==='teen-patti'){
      ...
      state.deck=d.slice(players.length*3);state.boot=10;state.pot=players.length*10;state.turn=0;state.seen={};
  ...
  async function action(type:string){
    ...
    if(room.game==='teen-patti'){
      if(s.phase!=='playing'||s.turn!==players.findIndex(p=>p.player_id===me))return;
      if(type==='seen')s.seen[me]=true;
      if(type==='fold')s.folded=[...(s.folded||[]),me];
      if(type==='bet'){const amt=s.seen[me]?s.boot*2:s.boot;s.pot+=amt;s.bets[me]=(s.bets[me]||0)+amt}
      const alive=players.filter(p=>!(s.folded||[]).includes(p.player_id));
      if(alive.length===1){s.phase='finished';s.winner=alive[0].player_id}
      else{s.turn=(s.turn+1)%players.length;while((s.folded||[]).includes(players[s.turn].player_id))s.turn=(s.turn+1)%players.length;}
  ...
  return <main className="wrap">
    ...
    <div className="card table">
      ...
      {room?.state?.phase==='playing'&&!isKitti&&<>
        ...
        {room.game==='teen-patti'&&<div className="actions"><button className="btn secondary" onClick={()=>action('seen')}>👀 See cards</button><button className="btn primary" onClick={()=>action('bet')}>Chaal / Bet</button><button className="btn danger" onClick={()=>action('fold')}>Pack / Fold</button></div>}
      ...
      {room?.state?.phase==='finished'&&<div className="notice">🏆 Hand finished. Winner: {players.find(p=>p.player_id===st.winner)?.username||'—'}</div>}
