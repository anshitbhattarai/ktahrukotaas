...
export default function Table({params}:{params:Promise<{id:string}>}){
  ...
  useEffect(()=>{
    ...
    load();
    // Realtime gives instant updates; the short polling fallback keeps the
    // player list, coin balances, and Kitti ready status live even if
    // Supabase Realtime replication is not enabled for a table yet.
    const ch=supabase.channel('room-'+id)
      ...
      .on('postgres_changes',{event:'*',schema:'public',table:'players',filter:'id=eq.'+(localStorage.getItem('cr_player_id')||'')},load)
      .subscribe();
