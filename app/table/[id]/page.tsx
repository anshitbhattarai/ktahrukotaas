...
export default function Table({params}:{params:Promise<{id:string}>}){
  ...
  async function load(){
    ...
    if(r.data?.game==='kitti'){
      ...
      if(saved?.length===3)setKittiHands(saved); else setKittiHands([[],[],[]]);
    }
  }
  ...
  async function start(){
    ...
    if(room.game==='teen-patti'){
      ...
      await Promise.all(players.map(p=>supabase.from('room_players').update({coins:Math.max(0,(p.coins||0)-entry)}).eq('room_id',id).eq('player_id',p.player_id)));
      players.forEach((p,i)=>state.hands[p.player_id]=d.slice(i*9,i*9+9));
      state.deck=d.slice(players.length*9);state.kittiHands={};state.ready=[];state.round=0;state.entry=entry;state.pot=total;state.kittiResults=[];
      setKittiHands([[],[],[]]);setSelected([]);setActiveHand(0);
    }else{
      const sb=10,bb=20;
      const balances=await Promise.all(players.map(async p=>{const r=await supabase.from('players').select('coins').eq('id',p.player_id).single();return {id:p.player_id,coins:Number(r.data?.coins??0)}}));
      const dealer=((Number(room.state?.dealer??-1)+1)%players.length+players.length)%players.length;
      const sbIndex=players.length===2?dealer:(dealer+1)%players.length;
      const bbIndex=(sbIndex+1)%players.length;
      const needed=players.map((p,i)=>i===sbIndex?sb:i===bbIndex?bb:0);
      const short=balances.find((x,i)=>x.coins<needed[i]);
      if(short){setBusy(false);return setError(`${players.find(p=>p.player_id===short.id)?.username||'A player'} does not have enough coins for the poker blinds.`)}
      await Promise.all(balances.map((x,i)=>needed[i]?supabase.from('players').update({coins:x.coins-needed[i]}).eq('id',x.id):Promise.resolve()));
      await Promise.all(players.map(p=>supabase.from('room_players').update({coins:balances.find(x=>x.id===p.player_id)!.coins-needed[players.findIndex(q=>q.player_id===p.player_id)]}).eq('room_id',id).eq('player_id',p.player_id)));
      players.forEach((p,i)=>state.hands[p.player_id]=d.slice(i*2,i*2+2));
      state.deck=d.slice(players.length*2);state.community=[];state.stage='preflop';state.turn=sbIndex;state.pot=sb+bb;state.bets={};state.contributions={};state.stacks={};state.acted=[];state.currentBet=bb;state.folded=[];state.winner=null;state.revealed=false;state.showdown=[];state.tiedWinners=[];state.sb=sb;state.bb=bb;state.dealer=dealer;state.sbIndex=sbIndex;state.bbIndex=bbIndex;
      balances.forEach((x,i)=>{state.stacks[x.id]=x.coins-needed[i];state.contributions[x.id]=needed[i]});
    }
  ...
  async function action(type:string){
    ...
    if(room.game==='teen-patti'){
      ...
      if(s.ready.length===players.length){
        ...
        if(s.winner){
          ...
          if(winner){
            ...
            await supabase.from('players').update({coins:Number(current.data?.coins??0)+Number(s.pot||0)}).eq('id',winner.player_id);
            await supabase.from('room_players').update({coins:Number(winner.coins||0)+Number(s.pot||0)}).eq('room_id',id).eq('player_id',winner.player_id);
          }
        }
      }
      ...
      const idx=players.findIndex(p=>p.player_id===me);
      const folded=s.folded||[];
      const active=players.filter(p=>!folded.includes(p.player_id));
      if(!active.length)return;
      if(s.turn!==idx||s.phase!=='playing')return;
      const id=me;
      s.bets=s.bets||{};s.contributions=s.contributions||{};s.stacks=s.stacks||{};s.acted=s.acted||[];
      const contrib=Number(s.contributions[id]||0), stack=Number(s.stacks[id]||0), current=Number(s.currentBet||0);
      const payout=async(winners:string[])=>{
        const unique=[...new Set(winners)]; if(!unique.length)return;
        const pot=Number(s.pot||0), share=Math.floor(pot/unique.length), remainder=pot%unique.length;
        for(let i=0;i<unique.length;i++){const wid=unique[i];const q=await supabase.from('players').select('coins').eq('id',wid).single();const add=share+(i===0?remainder:0);await supabase.from('players').update({coins:Number(q.data?.coins??0)+add}).eq('id',wid);const rp=await supabase.from('room_players').select('coins').eq('room_id',id).eq('player_id',wid).single();await supabase.from('room_players').update({coins:Number(rp.data?.coins??0)+add}).eq('room_id',id).eq('player_id',wid)}
      };
      const advanceOrStreet=async()=>{
        const live=players.filter(p=>!(s.folded||[]).includes(p.player_id));
        if(live.length===1){
          s.winner=live[0].player_id;s.tiedWinners=[];s.revealed=true;s.showdown=live.map(p=>({id:p.player_id,r:bestFive([...(s.hands[p.player_id]||[]),...(s.community||[])])}));s.phase='finished';await payout([live[0].player_id]);return;
        }
        const allMatched=live.every(p=>Number(s.contributions[p.player_id]||0)===Number(s.currentBet||0) || Number(s.stacks[p.player_id]||0)===0);
        const allActed=live.every(p=>s.acted.includes(p.player_id) || Number(s.stacks[p.player_id]||0)===0);
        if(allMatched&&allActed){
          ...
          else{s.stage='showdown';const ranked=live.map(p=>({id:p.player_id,r:bestFive([...(s.hands[p.player_id]||[]),...(s.community||[])])})).sort((a,b)=>cmp(b.r,a.r));const top=ranked[0];const tied=ranked.filter(x=>cmp(x.r,top.r)===0);s.winner=tied.length===1?top.id:null;s.tiedWinners=tied.length>1?tied.map(x=>x.id):[];s.showdown=ranked;s.revealed=true;s.phase='finished';await payout(tied.map(x=>x.id));return;}
          s.currentBet=0;s.bets={};s.contributions=Object.fromEntries(live.map(p=>[p.player_id,0]));s.acted=[];
          const first=s.stage==='preflop'?s.sbIndex:s.bbIndex;
          s.turn=first;
          return;
        }
        let next=(idx+1)%players.length,count=0;while((s.folded||[]).includes(players[next].player_id)||Number(s.stacks[players[next].player_id]||0)===0){next=(next+1)%players.length;if(++count>players.length)break;}s.turn=next;
      };
      if(type==='fold'){s.folded=[...folded,id];s.acted=s.acted.filter((x:string)=>x!==id);await advanceOrStreet()}
      else if(type==='check'){
        if(contrib!==current)return setError(`You must call ${current-contrib} coins or fold.`);
        s.acted=[...new Set([...s.acted,id])];await advanceOrStreet();
      }else if(type==='call'){
        const amount=Math.min(Math.max(0,current-contrib),stack);if(amount<=0)return setError('There is nothing to call.');s.stacks[id]=stack-amount;s.contributions[id]=contrib+amount;s.bets[id]=(s.bets[id]||0)+amount;s.pot+=amount;s.acted=[...new Set([...s.acted,id])];await advanceOrStreet();
      }else if(type==='raise'){
        const newBet=current+20;const amount=Math.min(Math.max(0,newBet-contrib),stack);if(amount<=0)return setError('Not enough coins to raise.');s.stacks[id]=stack-amount;s.contributions[id]=contrib+amount;s.bets[id]=(s.bets[id]||0)+amount;s.pot+=amount;s.currentBet=Math.max(current,contrib+amount);s.acted=[id];await advanceOrStreet();
      }
    }
    await saveState(s);
  }
  ...
  function autoOrder(){
    ...
    if(cards.length!==9)return setError('Your 9 cards are not ready yet.');
    setError('');
    let best:Card[][]|null=null;
    let bestScore:number[]|null=null;
    const score=(groups:Card[][])=>{
      const rs=groups.map(g=>teenRank(g)).sort((a,b)=>cmp(b,a));
      return [...rs[0],-999,...rs[1],-999,...rs[2]];
    };
    ...
    for(let a=0;a<9;a++)for(let b=a+1;b<9;b++)for(let c=b+1;c<9;c++){
      ...
      const rest=cards.map((x,i)=>({x,i})).filter(v=>!first.has(v.i));
      for(let i=0;i<6;i++)for(let j=i+1;j<7;j++)for(let k=j+1;k<8;k++){
        const second=[rest[i].x,rest[j].x,rest[k].x];
        const secondIds=new Set([rest[i].i,rest[j].i,rest[k].i]);
        const third=rest.filter(v=>!secondIds.has(v.i)).map(v=>v.x);
        if(third.length!==3)continue;
        const groups=[[cards[a],cards[b],cards[c]],second,third].sort((x,y)=>cmp(teenRank(y),teenRank(x)));
        const sc=score(groups);
        if(!bestScore||better(sc,bestScore)){bestScore=sc;best=groups;}
      }
    }
    if(best){
      setKittiHands(best);
  ...
  function pokerRankLabel(cards:Card[]){
    if(cards.length<5)return 'Waiting for 5 cards';
    const r=bestFive(cards);
    return ['High Card','Pair','Two Pair','Three of a Kind','Straight','Flush','Full House','Four of a Kind','Straight Flush'][r[0]]||'Hand';
  }
  ...
  return <main className="wrap">
    ...
    <div className="card table">
      ...
      {room?.host_id===me&&room?.state?.phase==='finished'&&<div className="actions"><button className="btn primary" onClick={start} disabled={busy}>🔄 Play Again</button><span className="muted">Starts a fresh {room.game==='teen-patti'?'Teen Patti':room.game==='kitti'?'Kitti':'Poker'} round.</span></div>}
      ...
      {room?.state?.phase==='playing'&&!isKitti&&room.game!=='teen-patti'&&<>
        ...
        {room.game==='poker'&&<><h3>Community · {String(st.stage||'preflop').toUpperCase()}</h3><div className="cards">{(st.community||[]).map((c:Card,i:number)=><div className={`cardface ${c.s===1||c.s===2?'red-suit':''}`} key={i}>{label(c)}</div>)}</div><p className="muted">Blinds: 🪙 {st.sb||10}/{st.bb||20} · Current bet: 🪙 {st.currentBet||0} · Your total this street: 🪙 {st.contributions?.[me]||0}</p><div className="actions">{st.turn===players.findIndex(p=>p.player_id===me)&&<><button className="btn secondary" onClick={()=>action('check')} disabled={Number(st.contributions?.[me]||0)!==Number(st.currentBet||0)}>Check</button><button className="btn primary" onClick={()=>action('call')}>Call {Math.max(0,Number(st.currentBet||0)-Number(st.contributions?.[me]||0))}</button><button className="btn warn" onClick={()=>action('raise')}>Raise +20</button><button className="btn danger" onClick={()=>action('fold')}>Fold</button></>} </div><p className="muted">{st.turn===players.findIndex(p=>p.player_id===me)?'Your turn.':'Waiting for '+(players[st.turn]?.username||'player')+'…'}</p></>}
      </>}
      ...
      {room?.state?.phase==='finished'&&!isKitti&&<div className="notice"><b>{st.winner?`🏆 Winner: ${players.find(p=>p.player_id===st.winner)?.username||'—'}`:st.tiedWinners?.length>1?`🤝 Tie: ${st.tiedWinners.map((id:string)=>players.find(p=>p.player_id===id)?.username||id).join(', ')}`:'🏁 Hand finished'}</b>{st.revealed&&st.showdown?.length>0&&<><div className="showdown"><h3>Showdown — all cards revealed</h3>{st.showdown.map((x:any)=><div className="show-row" key={x.id}><b>{players.find(p=>p.player_id===x.id)?.username||'Player'}</b><span>{(st.hands?.[x.id]||[]).map((c:Card)=>label(c)).join('  ')}</span><span>{room.game==='poker'?pokerRankLabel([...(st.hands?.[x.id]||[]),...(st.community||[])]) : rankLabel(st.hands?.[x.id]||[])}</span></div>)}</div></>}</div>}
